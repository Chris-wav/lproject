...
const SummonerCard = () => {
  const { xp, streak } = useGameContext();
    ...
    <div className="bg-[#1A1A2E] rounded-[20px] p-5 w-[90%] mx-auto mt-2 relative overflow-hidden text-white">
      ...
      <div className="flex items-center gap-4 relative z-10 mb-5">
        ...
        <div className="flex-1">
          <p className="font-display font-[800] text-lg">{profile?.username}</p>
          <p className="text-[#0BC4E3] text-xs font-semibold mt-1">⚔️ Gold Strategist</p>
        </div>
      ...
      <div className="relative z-10">
        <div className="flex justify-between text-xs text-white/50 mb-2">
          <span>SUMMONER LEVEL {Math.floor(xp / 1000)}</span>
          <span>{xp} / 1000 XP</span>
        </div>
        <div className="bg-white/10 rounded-full h-2">
          <div className="bg-[#0BC4E3] h-2 rounded-full" style={{ width: `${(xp % 1000) / 10}%` }} />
        </div>
        <div className="mt-2 inline-flex items-center gap-1 bg-[#0BC4E3]/15 rounded-full px-3 py-1">
          <span className="text-[#0BC4E3] text-xs font-bold">⚡ {1000 - (xp % 1000)} XP to next level</span>
        </div>
      </div>
    </div>
  );
