  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div className="bg-white rounded-2xl p-5">
        <p className="font-bold text-[#2C2A3A] text-base">{question.question}</p>
      </div>
      <div className="flex flex-col gap-3">
        {question.options.map((option, index) => {
          ...
          return (
            <button key={index} onClick={() => !hasAnswered && onAnswer(index)}
      ...
      {hasAnswered && (
        <div className={`rounded-2xl p-4 ${isCorrect ? "bg-[#E8F8F0] border border-[#1DB87A]" : "bg-[#FFF0F0] border border-[#E24B4A]"}`}>
          <p className={`font-black text-sm mb-1 ${isCorrect ? "text-[#1DB87A]" : "text-[#E24B4A]"}`}>
